...
from mmf.datasets.processors import GloVeProcessor
from mmf.common.sample import Sample as samp
from mmf.common.sample import SampleList as samplist
...
#list used for getting glove embeddings
...
class VisualBERTBase(BertPreTrainedModel):
    ...
    def get_final_embeddings(self,text_length,embedding_output,glove_embeddings)->Tensor:
        
        text_embedding = embedding_output[:, :text_length, :]
        visual_embedding = embedding_output[:, text_length:, :]
        bert_embed=[]
        pca_bert = PCA(n_components=128)
        for text_embed in text_embedding.cpu().detach().numpy():
          df = pd.DataFrame(text_embed)
          X_new = pca_bert.fit_transform(df)
          twod_list=[]
          for it in X_new:
            strFormat = len(it) * '{:.4f} '
            formattedList = strFormat.format(*it)
            newfeatlist=[float(i) for i in formattedList.split()]    
            twod_list.append(newfeatlist)
          
          bert_embed.append(np.array(twod_list))
        
        
        bert_embeddings=torch.tensor(np.array(bert_embed),dtype=torch.float32,)
        final_text_embed = torch.cat((bert_embeddings,glove_embeddings),2)
        final_text_embeddings = F.interpolate(final_text_embed, size=768)
        final_text_embeddings = final_text_embeddings.type(torch.float)
        print(final_text_embeddings)
        
        embedding_output=torch.cat((final_text_embeddings.to(visual_embedding.device),visual_embedding),1)
        #embedding_output=embedding_output.apply_(lambda x:float(format(x, '.4f')))
    ...
    def forward(
        ...
        embedding_output = self.embeddings(glove_embeddings,
            input_ids,
            token_type_ids,
        ...
        #visual_embedding = embedding_output[:, text_length:, :]
        #embedding_output=self.get_final_embeddings(text_length,embedding_output,glove_embeddings)
        #embedding_output[:, :text_length, :]=final_text_embeddings.to(visual_embedding.device)
        #embedding_output=embedding_output.cpu().detach().apply_(lambda x:float(format(x, '.4f')))
        #embedding_output=embedding_output.to(visual_embedding.device)
        #print('This is after ',embedding_output)
      
        #embedding_output_with_expo = torch.cat((final_text_embeddings.to(visual_embedding.device),visual_embedding),1)
        
        #print("These re final embedding for feeding ",embedding_output_with_expo)
        #final_tensor_embedding=torch.tensor(final_list_embedding)
        #embedding_output=final_tensor_embedding.to(visual_embedding.device)
...
class VisualBERTForClassification(nn.Module):
    ...
    def forward(
        ...
        sequence_output, pooled_output, attention_weights = self.bert(glove_embeddings,
            input_ids,
            attention_mask,
            token_type_ids,
            visual_embeddings,
...
class VisualBERT(BaseModel):
    ...
    def build(self):
        ...
        self.language_module = GloVeProcessor(self.config.text_processor.params)
    ...
    def get_glove_embeddings(self,sample_list: Dict[str, Tensor])->Tensor:
        glove_embed=[]  
        tokens_ = sample_list["tokens"]
        input_ids=sample_list['input_ids']
        for tok in tokens_:
          items = {"tokens":tok}
          gloved=self.language_module(items)
          g_vec=gloved['text']
          glove_embed.append(g_vec.tolist())
          
        glove_embeddings=torch.tensor(np.array(glove_embed),dtype=torch.float32,device=input_ids.device)
    ...
    def forward(self, sample_list: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        glove_embeddings=self.get_glove_embeddings(sample_list)
        ...
        output_dict = self.model(glove_embeddings,
